...
@testset "Infeasibility Diagnostics Tests" begin
    ...
    @testset "IISResult Struct Tests" begin
        ...
        @testset "Empty conflicts vector" begin
            result = IISResult(status = MOI.CONFLICT_FOUND, conflicts = IISConflict[])
        ...
        @testset "Multiple conflicts" begin
            conflicts = [
                ...
                IISConflict(
                    constraint_ref = nothing,
                    constraint_name = "bounds[y]",
                    expression = "0 <= y <= -1",
                ),
            ]

            result = IISResult(status = MOI.CONFLICT_FOUND, conflicts = conflicts)
    ...
    @testset "write_iis_report Function Tests" begin
        ...
        @testset "Custom filename" begin
            ...
            report_path =
                write_iis_report(result; output_dir = output_dir, filename = custom_name)
        ...
        @testset "Not found status report" begin
            result = IISResult(status = MOI.NO_CONFLICT_FOUND, solver_used = "HiGHS")
