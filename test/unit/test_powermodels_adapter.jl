...
@testset "PowerModels Adapter Tests" begin
    ...
    function create_test_hydro_plant(; id::String, bus_id::String)
        ReservoirHydro(;
            ...
            storage_capacity_m3=1e9,
            initial_storage_m3=5e8,
            max_turbine_outflow_m3_per_s=1000.0,
            productivity_mw_per_m3_s=1.0
        )
    end
    ...
    @testset "Bus Conversion" begin
        ...
        @testset "Area and zone hashing" begin
            ...
            @test pm_bus["area"] isa Int
            @test pm_bus["zone"] isa Int
            @test pm_bus["area"] >= 0
            @test pm_bus["zone"] >= 0
        end
    end
