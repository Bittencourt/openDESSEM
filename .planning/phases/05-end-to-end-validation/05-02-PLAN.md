...
Note: This plan depends on Plan 01 (wave 2). The types from `src/validation/validation_types.jl` are guaranteed to exist. Include them directly in the test file.
...
Actual DataFrame schema from `get_thermal_dispatch_dataframe()` / `get_hydro_dispatch_dataframe()` (Plan 03 helpers): columns `plant_id`, `period`, `generation_mw`.
Reference DataFrame schema from reference loaders: columns `period`, `plant_id`, `generation_mw`.

Both DataFrames use the same column name `generation_mw`, so join directly on `:plant_id, :period` and compare the `generation_mw` columns. The helpers in Plan 03 standardize the column names from the raw solution extraction API.
