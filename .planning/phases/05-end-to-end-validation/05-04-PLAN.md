...
2. **Test: Run validation against reference data** (VALD-02, VALD-03):
   ...
   @testset "Validation Against Reference" begin
       ...
       # Log validation results with comparison deltas for diagnostic visibility
       println("=== Validation Results ===")
       println("Overall: ", vr.passed ? "PASS" : "FAIL")
       println("Cost: ", vr.cost_passed ? "PASS" : "FAIL")
       if !isnothing(vr.cost_comparison)
           println("  Expected: ", vr.cost_comparison.expected)
           println("  Actual:   ", vr.cost_comparison.actual)
           println("  Rel Diff: ", round(vr.cost_comparison.relative_diff * 100; digits=2), "%")
       end
       println("PLD: ", vr.pld_passed ? "PASS" : "FAIL")
       if !isnothing(vr.pld_comparison)
           println("  Pass Rate: ", round(vr.pld_comparison.pass_rate * 100; digits=1), "%")
           println("  Threshold: ", round(vr.pld_comparison.threshold * 100; digits=1), "%")
       end
       println("Dispatch: ", vr.dispatch_passed ? "PASS" : "FAIL")
       if !isempty(vr.dispatch_comparisons)
           n_pass = count(mc -> mc.status == :passed, vr.dispatch_comparisons)
           println("  Plants Passing: ", n_pass, "/", length(vr.dispatch_comparisons))
       end
       println("Exit code: ", vr.exit_code)

       # Guarded hard assertions: only enforce when baseline is established
       # Enable via: OPENDESEM_ASSERT_VALIDATION=true
       if get(ENV, "OPENDESEM_ASSERT_VALIDATION", "false") == "true"
           @test vr.cost_passed   # VALD-02: cost within tolerance
           @test vr.pld_passed    # VALD-03: PLD pass rate meets threshold
       end
   end
   ```
...
**Update `.planning/ROADMAP.md` Phase 5 success criterion 3:**
The current criterion says "correlation > 0.85" which contradicts the CONTEXT.md locked decision to use pass-rate threshold approach (NOT correlation). Update it to:
```
3. Per-submarket PLD marginal prices match official DESSEM using pass-rate threshold approach (default: 80% of periods within 10% tolerance, configurable)
```
This aligns ROADMAP.md with the locked decision in CONTEXT.md.
  </action>
  <verify>
