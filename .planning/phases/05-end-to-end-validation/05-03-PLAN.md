...
5. **`validate_against_reference()` orchestrator:**
   ...
   - Extracts actual data from SolverResult using existing APIs: `get_cost_breakdown(result, system).total` for cost, `get_pld_dataframe(result)` for PLD, `get_thermal_generation(result)` and `get_hydro_generation(result)` for dispatch
   ...
   - Note: This function depends on Solvers module types. Import `SolverResult` from `..Solvers` (same pattern as Analysis.jl)
...
The Validation module imports `SolverResult` from `..Solvers` and `ElectricitySystem` from `..Core`. Since Analysis already imports SolverResult from `..Solvers`, the parent module hierarchy should work. If there are import path issues, adjust to use the correct relative path (Validation is nested inside Analysis, so `...Solvers` may be needed for SolverResult - the path is `OpenDESSEM.Solvers.SolverResult` from Validation's perspective).
